...
_Last updated: 2026-07-25 (level-1 authored city layout)._
...
- **Level 1 is now an authored city, not a scatter** (`src/content/citylayout.js`, 2026-07-25): a deterministic seeded street grid (5 roads each direction, 90u wide, 420u spacing) with zoned blocks — 9 downtown office towers, 15 residential apartment mid-rises, storefront-lined mixed blocks, a park (trees + benches) beside the spawn intersection, a parking lot (parked-car rows + speed bumps), and a plaza block that anchors the landmark. Street furniture throughout: streetlights, hydrants, mailboxes, street trees, curbside-parked cars in real paint colors, buses as traffic, and litter on both sidewalks and asphalt. All 7 template tiers keep their exact counts/radius/mass (building-medium renders as `apartment`, building-large as `office`), so balance is unchanged; the new furniture kinds (`tree`/`streetlight`/`bench`/`mailbox`/`hydrant`/`speed-bump`) are tuned to the starting size gate. New prop builders live in `src/content/propkit.js`, which also exports `scaleForRadius(kind, radius)` — level-1 meshes are uniformly scaled so their VISUAL size matches their swallow radius (the raw prop meshes are 1-15 units vs gameplay radii 16-90; with an authored grid the mismatch was glaring). New Skyline-opedia quips in `src/meta/collection.js`. Levels 2-100 keep the zoned-scatter placement. Verified in a real browser (Playwright, local server): level 1 boots with zero console errors, blind driving eats immediately, level 2 regression clean. NOTE: port 3003 on this machine may be occupied by a server for the separate `flywheel-v2` project — serve this repo with `PORT=3010 npm start` for local verification.
- **60-check headless test suite** (`npm test` / `scripts/logic-test.js`, rewritten this session) — no longer regex-extracts a `<script>` tag; imports the real `src/` modules directly under plain Node. Covers formulas, level generation/pacing invariant, save/load (with and without localStorage), upgrades, real Three.js scene-graph construction (props/landmarks, no GPU needed), the level-1 city layout (determinism, template count/mass preservation, bounds, furniture presence, off-road buildings, mesh validity), swallow math, achievements/combo, and audio. **60 passed, 0 failed**, confirmed by direct run.
