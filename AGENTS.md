...
- `src/engine/` — `scene.js` (renderer/camera/clock, plus the postprocessing chain: `EffectComposer` + `BokehPass` depth-of-field whose focus main.js pins to the camera→avatar distance each frame; addons are version-matched copies from `node_modules/three/examples/jsm` vendored under `assets/vendor/postprocessing/` + `assets/vendor/shaders/`), `avatar.js` (player growth/movement, `radius() = 26 + sqrt(mass)*1.9`), `camera.js` (chase cam with obstacle-avoidance raycast), `input.js` (keyboard/pointer → normalized axes; main.js rotates them by camera yaw — `rotateAxesByYaw` — so WASD is camera-relative).
